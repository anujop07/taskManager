      ...
      <div className="task-content">
        <div className="task-details">
          <div className="task-title">
            {post.title}
            <span className="priority-badge priority-medium">{post.category}</span>
          </div>
          {post.content && <div className="task-desc">{post.content}</div>}
          {post.author && <div className="task-desc" style={{ marginTop: '0.25rem' }}>✍️ {post.author}</div>}
        </div>
      </div>
      <div className="task-actions">
        <button className="icon-btn" onClick={() => onEdit(post)} title="Edit Post">
        ...
        <button className="icon-btn delete" onClick={() => onDelete(post._id)} title="Delete Post">
